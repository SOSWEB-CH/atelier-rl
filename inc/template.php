...
print "<pre>alt:";
print "<br>Info:";
//$INFO['meta']['description'] = array("texte court de présentation du site");
//$INFO['meta']['subject'] = array("hello boys","hello girls");
print_r($INFO);
print "</pre>";
//  */
  // the usual stuff
  ...
  /// ### start patch by Alain - 2007-06-04
    // description (explicit or implicit)
    if(!empty($INFO['meta']['subject'])){
	  if( is_array($INFO['meta']['subject']) ) {
        $head['meta'][] = array( 'name'=>'description', 'content'=>join(',',$INFO['meta']['subject']));
	  } else {
	    $head['meta'][] = array( 'name'=>'description', 'content'=> $INFO['meta']['subject']);
	  }
    }else{
      $head['meta'][] = array( 'name'=>'description', 'content'=>str_replace(':',',',$ID));
    }

    // keywords (explicit or implicit)
    if(!empty($INFO['meta']['keywords'])){
	  if( is_array($INFO['meta']['keywords']) ) {
        $head['meta'][] = array( 'name'=>'keywords', 'content'=>join(',',$INFO['meta']['keywords']));
	  } else {
	    $head['meta'][] = array( 'name'=>'keywords', 'content'=> $INFO['meta']['keywords']);
	  }
    }else{
      $head['meta'][] = array( 'name'=>'keywords', 'content'=>str_replace(':',',',$ID));
    }
  }
  /// ### end patch by Alain - 2007-06-04
  // load stylesheets
